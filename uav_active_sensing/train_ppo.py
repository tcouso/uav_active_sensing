...
from uav_active_sensing.img_env.img_exploration_env import (
    RewardFunction,
    ImageExplorationEnv,
...
class ImageEnvFactory:
    def __init__(self, log_dir: str, env_config: ImageExplorationEnvConfig):
...
def train_ppo(params: dict, experiment_name: str = None, nested: bool = False) -> dict:
    ...
    with mlflow.start_run(nested=nested):
        ...
        mae_model = ViTMAEForPreTraining.from_pretrained(
            "facebook/vit-mae-base", config=mae_config
        ).to(DEVICE)

        act_mae_config = ActViTMAEForPreTraining.config_class.from_pretrained(
            "facebook/vit-mae-base"
        )
        act_mae_config.seed = seed
        act_mae_model = ActViTMAEForPreTraining.from_pretrained(
            "facebook/vit-mae-base", config=act_mae_config
        ).to(DEVICE)

        reward_function = RewardFunction(
            act_mae_model,
            num_agents=params["num_samples"],
        ...
        train_vec_env = DummyVecEnv(
            [factory(i, dataset=dataset_list[i]) for i in range(params["num_envs"])]
        )
        ...
        return {"loss": -mean_reward, "loss_variance": -std_reward, "status": STATUS_OK}
