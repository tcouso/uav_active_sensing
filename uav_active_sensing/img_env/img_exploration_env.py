...
# TODO: Fix elevation dynamics. Relate elevation with a lens dynamics model


# TODO: Multiple agent support (https://pettingzoo.farama.org/)
...
class RewardFunction:
    def __init__(
        ...
        self.last_rewards = torch.zeros(self.num_agents, dtype=torch.float32)

    def __call__(self, img: torch.Tensor, sampled_img: torch.Tensor) -> torch.Tensor:

        rewards = torch.zeros(self.num_agents, dtype=torch.float32)

        # TODO: Join this pass in one tensor
        for i in range(self.num_agents):
            ...
            with torch.no_grad():
                outputs = self.model(img.unsqueeze(0), masked_img.unsqueeze(0))
            loss = outputs.loss
            reward_i = 1 / (1 + loss)
            rewards[i] = reward_i
        ...
        if self.reward_increase:
            rewards = rewards - self.last_rewards
    ...
    def sampled_img_random_masking(self, sampled_img: torch.Tensor) -> torch.Tensor:
        ...
        selected_patches = tuple(
            idx[mask_indices] for idx in valid_indices
...
class ImageExplorationEnv(gym.Env):
    ...
    def __init__(self, dataset: Dataset, seed: int, env_config: ImageExplorationEnvConfig) -> None:
        ...
        self._sampled_kernel_size_mask = torch.full_like(
            self.img, fill_value=self.img_h // self.sensor_h, dtype=torch.int32
        )  # Max possible kernel size
    ...
    def _get_obs(self) -> Dict[np.ndarray, np.ndarray]:
        ...
        obs = {"sampled_img": sampled_img, "pos": pos}
    ...
    def reset(
        ...
        self._kernel_size = 1
        self.reward_function.last_rewards = 0
        self._step_count = 0
    ...
    ) -> Tuple[Dict[np.ndarray, np.ndarray], float, bool, bool, Dict]:
    ...
    def fov_bbox(self) -> torch.Tensor:
        ...
        offset = torch.tensor(
            [self.sensor_h * self._kernel_size, self.sensor_w * self._kernel_size]
        )
        # print(offset)
    ...
    def sensor_max_pos_from_kernel_size(self) -> torch.Tensor:
        ...
        sensor_max_pos = torch.tensor(
            [self.img_h - self.sensor_h, self.img_w - self.sensor_w], dtype=torch.int32
        )
    ...
    def _sensor_pos(self, new_position: torch.Tensor) -> None:
        ...
        new_position = torch.clamp(
            new_position, min=self._sensor_min_pos, max=self.sensor_max_pos_from_kernel_size
        )
    ...
    def _kernel_size(self, new_kernel_size: int) -> None:
        ...
        new_kernel_size = max(
            self._min_kernel_size, min(new_kernel_size, self.max_kernel_size_from_sensor_pos)
        )
